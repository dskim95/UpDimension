...
namespace UpDimension
{
    public partial class Form1 : Form
    {
        ...
        string fileName
        {
            ...
            {
                filename = value;
                this.Text = "UpDimension" + " [" + filename + "]";
            }
        }
        int pointnumber;
        ...
        Line highlightLine;
        List<Line> highlightLineList;
        PointF highlightPoint;
        int highlightPointNumber;
        PointF mousePoint;
        // Line의 끝점이나 교점이면 true
        bool highlightLinePoint;
        Face highlightFace;

        bool lineDraw;
        Line savedLine;
        Line bufferedImageLine;
        bool mouseDown;
        bool pointMove;
        Vertex2D savedVertex;
        Face bufferedFace;
        ...
        private void displayPointNumber(int pointnumber)
        {
        ...
        {
            this.openButton.FlatAppearance.BorderColor = Color.FromArgb(0, 255, 255, 255);
            this.saveButton.FlatAppearance.BorderColor = Color.FromArgb(0, 255, 255, 255);
            this.sceneButton.FlatAppearance.BorderColor = Color.FromArgb(0, 255, 255, 255);
            this.exportButton.FlatAppearance.BorderColor = Color.FromArgb(0, 255, 255, 255);
        ...
        private void pictureBox1_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            if (this.vanishingLineButton.Checked)
            {
                bufferedImageLine.P1 = currentScene.picture2Image(e.Location);
                bufferedImageLine.P2 = currentScene.picture2Image(e.Location);
                lineDraw = true;
            }
            else if (this.guideLineButton.Checked)
            {
                if (highlightPoint.X != -1 && e.Button == MouseButtons.Right)
                {
                    if (highlightLine.P1.equal(highlightPoint) || highlightLine.P2.equal(highlightPoint))
                    {
                        if (highlightLine.P1.equal(highlightPoint))
                            bufferedImageLine.P1 = highlightLine.P2;
                        else if (highlightLine.P2.equal(highlightPoint))
                            bufferedImageLine.P1 = highlightLine.P1;
                        bufferedImageLine.P2 = currentScene.picture2Image(e.Location);
                ...
                else if (e.Button == MouseButtons.Left)
                {
                    ...
                    {
                        bufferedImageLine.P1 = highlightPoint;
                        bufferedImageLine.P2 = highlightPoint;
                    }
                    // highlightLine이 있으면 현재 위치에서 line에 내린 수선의 발을 시작점으로
                    else if (highlightLine != null)
                    {
                        bufferedImageLine.P1 = highlightLine.projection(currentScene.picture2Image(e.Location));
                        bufferedImageLine.P2 = highlightLine.projection(currentScene.picture2Image(e.Location));
                    }
                    else
                    {
                        bufferedImageLine.P1 = currentScene.picture2Image(e.Location);
            ...
            else if (guideLineButton.Checked)
            {
                ...
                if (lineDraw)
                {
                    if (highlightPoint.X != -1)
                        ...
                        bufferedImageLine.P2 = highlightLine.projection(currentLocation);
                    else
                        bufferedImageLine.P2 = currentLocation;
                }
        ...
        private void pictureBox1_MouseUp(object sender, MouseEventArgs e)
        {
            ...
            {
                if (radioButtonX.Checked)
                    currentScene.addVanishingLine(bufferedImageLine, Scene.X);
                else if (radioButtonY.Checked)
                    currentScene.addVanishingLine(bufferedImageLine, Scene.Y);
                else if (radioButtonZ.Checked)
                    currentScene.addVanishingLine(bufferedImageLine, Scene.Z);
                else
                    currentScene.addGuideLine(bufferedImageLine);
                lineDraw = false;
                savedLine = null;
            ...
            else if (faceButton.Checked && e.Button == MouseButtons.Left)
            {
                ...
                if (Scene.closestPoint(currentLocation, currentScene.vertexList, ref min, ref closestVertex))
                {
                    if (bufferedFace == null)
                        bufferedFace = new Face();
                    bufferedFace.vertexList.Add(closestVertex.number);
                }
            ...
            {
                if (bufferedFace != null && bufferedFace.Count >= 2)
                {
                    model.face.Add(bufferedFace);
                    changePictureBoxImage(currentScene.display());
                }
                bufferedFace = null;
            }
        ...
        private void pictureBox1_Paint(object sender, PaintEventArgs e)
        {
            ...
            if (currentScene != null)
                bufferedPictureLine = currentScene.imageLine2PictureLine(bufferedImageLine);
            e.Graphics.SmoothingMode = System.Drawing.Drawing2D.SmoothingMode.AntiAlias;
            // bufferedPictureLine을 그림
            if (this.vanishingLineButton.Checked && lineDraw)
            {
                if (this.radioButtonX.Checked)
                    new vLine(bufferedImageLine, vLine.Xaxis).draw(e.Graphics, currentScene.currentImage, currentScene.image2Picture);
                else if (this.radioButtonY.Checked)
                    new vLine(bufferedImageLine, vLine.Yaxis).draw(e.Graphics, currentScene.currentImage, currentScene.image2Picture);
                else if (this.radioButtonZ.Checked)
                    new vLine(bufferedImageLine, vLine.Zaxis).draw(e.Graphics, currentScene.currentImage, currentScene.image2Picture);
            }
            ...
            else if (faceButton.Checked)
            {
                ...
                if (bufferedFace != null && bufferedFace.Count >= 2)
                {
                    foreach (int item in bufferedFace.vertexList)
                    {
                        PointF point = currentScene.image2Picture(currentScene.vertexList[item].coord);
                        e.Graphics.FillEllipse(new SolidBrush(Color.White), point.X - 3F, point.Y - 3F, 6F, 6F);
                    }
                    e.Graphics.FillPolygon(new SolidBrush(Color.FromArgb(128, 192, 192, 192)), currentScene.face2PointArray(bufferedFace));
                    e.Graphics.DrawPolygon(new Pen(Brushes.Black, 2), currentScene.face2PointArray(bufferedFace));
                }
            }
